...
Test Keep
```{r}
str(GB4$Communication_Category)

```

```{r}
# Ensure the categories are ordered if appropriate
...
# Convert Occupation and other categorical variables to factors
...
# Subset the variables you want from GB4
...
# Merge the subset of GB4 with completed_data
...
```{r}
# Replace the variables in the original dataset (GB4) with the imputed values
...
```{r}
# Ensure both datasets have the same common identifier column (e.g., "id")
# Replace "id" with your actual common identifier if different
if (!"id" %in% colnames(completed_data)) {
  stop("Make sure both datasets have a common identifier column, like 'id'")
}

# Merge completed_data with GB4
...
# Select relevant variables from GB4
GB5 <- GB4[, c(
  "id", 
  "Age", 
...
# Fit a model with interaction terms for maternal age and other variables
# Load necessary library
library(nnet)

# Fit a multinomial logistic regression model with interaction
...
# Calculate odds ratios, 95% CIs, and p-values
...
# Create a tidy data frame for reporting
...
# Save the table to a CSV file or view it
print(results_table)
